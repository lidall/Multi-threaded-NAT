...
void
cuckoohash::UDPFlow::apply(WritablePacket *p, bool direction, unsigned annos)
{
...
cuckoohash::cuckoohash() : _allocator()
{
}
...
void *
cuckoohash::cast(const char *n)
{
    if (strcmp(n, "IPRewriterBase") == 0)
	return (IPRewriterBase *)this;
    else if (strcmp(n, "cuckoohash") == 0)
	return (cuckoohash *)this;
    else
	return 0;
}

int
cuckoohash::configure(Vector<String> &conf, ErrorHandler *errh)
{
...
IPRewriterEntry *
cuckoohash::add_flow(int ip_p, const IPFlowID &flowid,
...
int
cuckoohash::process(int port, Packet *p_in)
{
    printf("%s\n", "hello from cgn:D");
    ...
    if (!m) {			// create new mapping
        ...
        if (result == rw_addmap) {
            m = cuckoohash::add_flow(ip_p, flowid, rewritten_flowid, port);
        }
...
void
cuckoohash::push(int port, Packet *p)
{
...
#if HAVE_BATCH
void
cuckoohash::push_batch(int port, PacketBatch *batch)
{
...
String
cuckoohash::dump_mappings_handler(Element *e, void *)
{
    cuckoohash *rw = (cuckoohash *)e;
